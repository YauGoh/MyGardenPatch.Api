...
{
    Task<TResult> HandleAsync<TQuery, TResult>(
        TQuery query, 
...
internal class QueryExecutor : IQueryExecutor
{
    ...
    public async Task<TResult> HandleAsync<TQuery, TResult>(
        TQuery query, 
        CancellationToken cancellationToken = default) where TQuery : IQuery<TResult>
    {
        var validator = _serviceProvider.GetRequiredService<IQueryValidator<TQuery>>()
            ?? throw new QueryValidatorNotFoundException<TQuery>();
...
public static class QueryExecutorExtensions
{
    ...
    public static Task<TResult> HandleAsync<TResult>(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var handleAsync = executor
            .GetType()
            .GetMethod(nameof(QueryExecutor.HandleAsync));

        if (handleAsync == null) throw new InvalidOperationException("");

        return (Task<TResult>)handleAsync
            .MakeGenericMethod(
                _lookup[typeof(TResult)], 
                typeof(TResult))
            .Invoke(
                executor, 
                new object?[] 
