...
namespace MyGardenPatch.GardenBeds.Commands
{
    ...
    public class AddPlantCommandHandler : ICommandHandler<AddPlantCommand>
    {
        ...
        public async Task HandleAsync(AddPlantCommand command, CancellationToken cancellationToken = default)
        {
            var gardenBed = await _gardenBeds.GetAsync(command.GardenBedId, cancellationToken);
