...
{
    public record DescribeGardenCommand(GardenId GardenId, string Name, string Description, Uri ImageUri, string ImageDescription)
        : IGardenCommand, INameableCommand, IImageableCommand;
    ...
    public class DescribeGardenCommandHandler : ICommandHandler<DescribeGardenCommand>
    {
        ...
        public async Task HandleAsync(DescribeGardenCommand command, CancellationToken cancellationToken = default)
        {
            var garden = await _gardens.GetAsync(command.GardenId);

            garden!.Describe(command.Name, command.Description, command.ImageUri, command.ImageDescription);
