...
{
    public record MoveGardenCommand(GardenId GardenId, Transformation Transformation, bool MoveGardenBeds) : IGardenCommand;
    ...
    public class MoveGardenCommandHandler : ICommandHandler<MoveGardenCommand>
    {
        ...
        public async Task HandleAsync(MoveGardenCommand command, CancellationToken cancellationToken = default)
        {
            var garden = await _gardens.GetAsync(command.GardenId, cancellationToken);

            garden!.Move(command.Transformation, command.MoveGardenBeds);
    ...
    {
        public MoveGardenCommandValidator(ICurrentUserProvider currentUser, IRepository<Garden, GardenId> gardens) : base(currentUser, gardens)
        {
        }
    }
}
